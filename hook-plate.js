(function(){
 ...
 function wrapExtra(digits,pairs,kind){
  ...
  if(hit.length){
   html+='<div class="hl-warn"><strong>課堂車牌筆記</strong>：生氣後面黐0（'+hit.join('、')+'）— 貴人越來越少，人際不長久，感情來得快去得快，友誼漾漾平淡，重複被騙；貴人唔給力。</div>';
  }
  var has409=false;
  if(pairs)pairs.forEach(function(p){if(p.display&&p.display.indexOf('409')>=0)has409=true;});
  if(has409||digits.indexOf('409')>=0){
   html+='<div class="hl-warn"><strong>課堂車牌筆記</strong>：409（天醫中間夾0）— 資產被套或沒法變現。</div>';
  }
  if(digits.indexOf('0')>=0){
   html+='<div class="hl-warn"><strong>課堂車牌筆記</strong>：車牌見0 — 隱藏地下情／隱藏婚姻，婚姻易有第三者；男易婚外情，女易被小三；亦易被朋友捅刀子。</div>';
  }
  if(pairs&&pairs.length){
   var has={};
   pairs.forEach(function(p){has[p.field]=1;});
   if(has.shengqi&&has.tianyi&&has.yannian){
    html+='<div class="hl-resolve"><strong>課堂車牌筆記</strong>：有齊生氣、天醫、延年三吉。';
   ...
   if(allTian){
    html+='<div class="nature"><strong>課堂車牌筆記</strong>：全程天醫（5夾中當伏跟前，未講增強），冇生氣同延年磁場。</div>';
   }
  }
  return html;
 }
